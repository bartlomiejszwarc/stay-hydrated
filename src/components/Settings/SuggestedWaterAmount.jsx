...
function SuggestedWaterAmount({ data }) {
  ...
  const [suggestedWaterIntakeAmount, setSuggestedWaterIntakeAmount] = useState(null);
  const calculateSuggestedWaterAmount = () => {
    const waterIntakeForMaleFormula = Math.round(10 * data.weight + 6.25 * data.height - 5 * data.age + 5);

    const waterIntakeForFemaleFormula = Math.round(10 * data.weight + 6.25 * data.height - 5 * data.age - 161);
    if (data?.gender === 'Male') {
      try {
